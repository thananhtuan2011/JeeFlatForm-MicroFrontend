...
import { ColumnWorkModel, UpdateWorkModel, UserInfoModel, WorkModel } from "../../../../models/jee-work.model";
import { MenuPhanQuyenServices, ProjectsTeamService, WeWorkService, WorkGeneralService } from "../../../../services/jee-work.service";
...
export class WorkListNewComponent implements OnInit, OnChanges {
  constructor(
    ...
    const user = this._DanhMucChungService.getAuthFromLocalStorage();
    if (user) {
  ...
  ngOnInit() {
    ...
    this.menuServices
      .GetRole("" + this.ID_Project)
      .pipe(
        map((res) => {
          if (res && res.status == 1) {
            // this.list_role = res.data.dataRole;
            this.list_role = res.data.Rule ? res.data.Rule : [];
            this.IsAdminGroup = res.data.IsAdmin;
          }
        })
      )
      .subscribe(() => {
    ...
    if (this.list_role) {
      if (this.list_role) {
        if (
          ...
          if (this.list_role.find((element) => element == 15)) {
            return false;
          }
        }
        if (RuleID === 10) {
          if (this.list_role.find((element) => element == 16)) {
            return false;
          }
        }
        if (RuleID === 4 || RuleID === 14) {
          if (this.list_role.find((element) => element == 17)) {
            return false;
          }
        }
        const r = this.list_role.find((element) => element == RuleID);
        if (r) {
          return true;
        } else {
          return false;
