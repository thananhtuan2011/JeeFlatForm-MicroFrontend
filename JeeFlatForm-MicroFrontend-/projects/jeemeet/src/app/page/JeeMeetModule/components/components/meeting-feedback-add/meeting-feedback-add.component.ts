...
@Component({
	selector: "app-meeting-feedback-add",
	templateUrl: "./meeting-feedback-add.component.html",
	changeDetection: ChangeDetectionStrategy.OnPush,
})


export class MeetingFeedbackAddComponent implements OnInit {
  ...
  strExtensionFile =
	...
	constructor(
		public dialogRef: MatDialogRef<MeetingFeedbackAddComponent>,
	...
	ngOnInit() {
		this.idRow = this.data._data.IdRow;
		this.isXem = this.data._data.isXem ? this.data._data.isXem : false;
		if (this.idRow > 0) {
			this.viewLoading = true;
			this.service.getDetailFeedback(this.idRow).subscribe((res) => {
		...
		if (this.idRow > 0) {
			if (!this.isXem) {
				return this.translate.instant("OBJECT.UPDATE.TITLE", {
					name: this.translate
						.instant("QL_CUOCHOP.CUOCHOP.FEEDBACK")
						.toLowerCase(),
				});
			}
			else {
				return this.translate.instant("OBJECT.DETAIL.TITLE", {
					name: this.translate
						.instant("QL_CUOCHOP.CUOCHOP.FEEDBACK").toLowerCase(),
				})
			}
		}
	...
	add(data) {
		this.viewLoading = true;
		this.service.addUpdateFeedback(data).subscribe((res) => {
			this.viewLoading = false;
			if (res.status == 1) {
				this.layoutUtilsService.showActionNotification(
					res.error.message,
	...
	prepare() {
		...
		_data.IdCuocHop = this.data._data.IdM;
