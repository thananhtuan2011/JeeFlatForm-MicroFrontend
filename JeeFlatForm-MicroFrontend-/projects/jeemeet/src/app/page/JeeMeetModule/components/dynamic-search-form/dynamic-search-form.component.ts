...
@Component({
    selector: 'meet-dynamic-search-form',
...
export class DynamicSearchFormComponent implements OnInit {
    ...
    createForm() {
        ...
        this.formSearch.controls["KeyView"].setValue("0")
    }
    ...
    showFormSearch() {
        ...
        this.searchForm.nativeElement.style.width = (Number(temp.width) + 330) + 'px';
    }
    ...
    GetListDV() {
        this.dynamicSearchFormService.GetListDonVi().subscribe(
            (res: any) => {
                if (res) {
