...
export class CuTagComponent implements OnInit {
  ...
  colorbg = "rgb(128, 0, 0)";
  constructor(
    private _service: DanhMucChungService,
    private layoutUtilsService: LayoutUtilsService,
  ) { }

  ngOnInit() {
    // this.colorbg = this.Opaciti_color(this.color)
    // if (this.tag.color=='#aaa') {
    //   this.tag.color='rgb(170, 170, 170)';
    // }
  }
  ...
  Update(_item: TagsModel, withBack: boolean) {
    this._service.UpdateTag(_item).subscribe(res => {
      if (res && res.status === 1) {
  ...
  Create(_item: TagsModel, withBack: boolean) {
    this._service.InsertTag(_item).subscribe(res => {
      if (res && res.status === 1) {
