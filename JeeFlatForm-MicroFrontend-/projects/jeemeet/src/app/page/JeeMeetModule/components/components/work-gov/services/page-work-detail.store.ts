...
export class PageWorkDetailStore  {
  ...
  private readonly wf_updateEvent = new BehaviorSubject<boolean>(false);
  readonly wfupdateEvent$ = this.wf_updateEvent.asObservable();

  get wfupdateEvent(): boolean {
    return this.wf_updateEvent.getValue();
  }

  set wfupdateEvent(val: boolean) {
    this.wf_updateEvent.next(val);
  }
