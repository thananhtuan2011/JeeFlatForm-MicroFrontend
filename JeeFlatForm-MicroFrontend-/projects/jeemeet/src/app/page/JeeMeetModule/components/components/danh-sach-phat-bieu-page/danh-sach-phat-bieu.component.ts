...
@Component({
	selector: 'app-danh-sach-phat-bieu-page',
	templateUrl: './danh-sach-phat-bieu.component.html',
	styleUrls: ['./danh-sach-phat-bieu.component.scss']
})
export class DanhSachPhatBieuPageComponent implements OnInit {
	@Input() idMeeting: number = 0;
	@Input() isThuKy: boolean = false;
	@Input() reload: boolean = false;
	@Output() phatBieu = new EventEmitter();
	@Output() callReload = new EventEmitter();
	_allData: any = [];
		...
		this.productsService.items$.subscribe(
			(res) => {
				if (res && res.length > 0) {
					...
					let userID = user['customData']["jee-account"].userID;
					var obj = res.find(user => user.UserID === userID);
					if (obj && obj.Status === 1) {
						this.phatBieu.emit(true)
					} else {
						this.phatBieu.emit(false)
					}
					this._allData = res.filter(user => user.Status === 1);
				} else {
					this._allData = [];
					this.phatBieu.emit(false)
				}
	...
	XinPhatBieu(UserID: any, type: any) {
		...
		this.productsService.XinPhatBieuCuocHop(item).subscribe(res => {
			if (res && res.status === 1) {
				...
				this.callReload.emit(!this.reload)
				this.loadDataList();
				this.changeDetectorRefs.detectChanges()
			} else {
				this.layoutUtilsService.showActionNotification(
