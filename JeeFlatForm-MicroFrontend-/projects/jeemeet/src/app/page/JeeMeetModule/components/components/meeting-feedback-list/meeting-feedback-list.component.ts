...
@Component({
  selector: 'app-meeting-feedback-list',
  templateUrl: './meeting-feedback-list.component.html'
})
export class MeetingFeedbackListComponent implements OnInit {
  ...
  constructor(
		public dialogRef: MatDialogRef<MeetingFeedbackListComponent>,
  ...
  paginate(paginator: PaginatorState) {
	...
	filterConfiguration(): any {
		...
		if (this.MeetingFeedback.IdM) {
			filter.IdM = this.MeetingFeedback.IdM;
		}
  ...
  xuatExcel()
  {
		this.productsService.exportExcelGopY(this.MeetingFeedback.IdM).subscribe((res:any) => {
			if (res && res.status == 1)
       {
