...
const root = '/ThongKeCuocHopDonViThamGia';
@Component({
	selector: "app-thong-ke-cuoc-hop-tham-gia-list",
	templateUrl: "./thong-ke-cuoc-hop-list.component.html",
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class ThongKeCuocHopThamGiaListComponent implements OnInit {
	...
	togglegroupfilter() {
		...
		const setCardbodyHeight = (elementId: string, height: number) => {
		...
		const tmp_height = window.innerHeight - 236;
		setCardbodyHeight("cardbody", tmp_height);
		setCardbodyHeight("cardbody2", tmp_height);

		const header = document.getElementById("cardbodyhead");
		const cardbody = document.getElementById("cardbody");
		const cardbody2 = document.getElementById("cardbody2");

		const height = window.innerHeight - header.clientHeight - (window.innerHeight - cardbody.clientHeight);
		const height2 = window.innerHeight - header.clientHeight - (window.innerHeight - cardbody2.clientHeight);

		if (this.filterTurn) {
			setCardbodyHeight("cardbody", height + 30);
			setCardbodyHeight("cardbody2", height2 - 130);
			this.changeDetectorRefs.detectChanges();
		} else {
			const tmp_height2 = window.innerHeight - 280;
			setCardbodyHeight("cardbody", tmp_height2);
			setCardbodyHeight("cardbody2", tmp_height2 + 50);
		}
	}
