...
const root = '/ThongKeCuocHopTheoChucVu';
@Component({
	selector: "app-thong-ke-cuoc-hop-theo-chuc-vu-list",
	templateUrl: "./thong-ke-cuoc-hop-list.component.html",
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class ThongKeCuocHopTheoChucVuListComponent implements OnInit {
	...
	GetListDV() {
		this.thongKeCuocHopService.GetDSChucDanh().subscribe(
			(res: any) => {
				if (res) {
					this.listIdDV = res.data;
					this.filteredDV.next(this.listIdDV);
					this.idsDV = []
					const user = this.getAuthFromLocalStorage()['user'];
					this.idsDV.push(Number(user["customData"]["personalInfo"].JobtitleID))
	...
	filterConfiguration(): any {
		...
		if (this.idsDV != "" && this.idsDV != undefined) {
			filter["idsChucVu"] = this.idsDV.join(",");
		}
		filter.tungay = this.datePipe.transform(this.startDate, "dd-MM-yyyy");
		filter.denngay = this.datePipe.transform(this.nowDate, "dd-MM-yyyy");
		return filter;
