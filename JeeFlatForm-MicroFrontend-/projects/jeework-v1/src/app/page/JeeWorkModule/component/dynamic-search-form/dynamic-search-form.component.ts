...
export class DynamicSearchFormComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        for (var i = 0; i < this.controls.length; i++) {
            ...
            if (this.controls[i].search) {
                let idx = i;
                this.dynamicSearchFormService.getInitDataSearch(this.controls[i].api).then(res => {
        ...
        for (var i = 0; i < this.controls.length; i++) {
            ...
            if (control.type != 5 && control.type != 8 && control.type != 11 && control.type != 12) {
                if (control.type == 3) {
                    ...
                    item[control.name] = this.delTextSearch ? '' :[control.default];
                    if (control.default != '' && control.default != undefined) {
                        this.dynamicSearchFormService.search_data$.next(control.default);
                    }
                } else {
                    item[control.name] = [''];
                }
    ...
    showFormSearch() {
        ...
        this.searchForm.nativeElement.style.left = (Number(temp.left) - 17) + 'px';
        // this.searchForm.nativeElement.style.width = temp.width + 'px' ;
        // this.searchForm.nativeElement.style.width = '600px' ;
