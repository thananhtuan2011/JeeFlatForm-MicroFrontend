...
import { WorkViewModel } from './jee-work.model';
...
const API_work_CU = environment.HOST_JEEWORK_API + '/api/tp-tasks';
...
const api_tp_task = environment.HOST_JEEWORK_API + '/api/tp-tasks-pc';
...
export class ProjectsTeamService {
    ...
    UpdateColumnWork(item: any): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        const url = api_work_click_up + '/update-column-work';
        return this.http.post<QueryResultsModel>(url, item, {
            headers: httpHeaders,
        });
    }
    ...
    UpdateColumnNewField(item: any): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        const url = api_work_click_up + '/update-column-new-field';//Thiên thao tác 10/10/2022
        return this.http.post<QueryResultsModel>(url, item, {
            headers: httpHeaders,
        });
    }
    ...
    UpdateNewField(item: any): Observable<QueryResultsModel> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        const url = api_work_click_up + '/update_new-field';//Thiên thao tác 10/10/2022
        return this.http.post<QueryResultsModel>(url, item, {
            headers: httpHeaders,
        });
    }
...
export class WeWorkService {
    ...
    list_assignee_task(): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		// let params = this.httpUtils.parseFilter(filter);
		return this.http.get<any>(api_tp_task + `/list-assignee-task`, { headers: httpHeaders });
	}
}
...
export class WorkService {
    ...
    _UpdateByKey(item: any): Observable<any> {
        const httpHeaders = this.httpUtils.getHTTPHeaders();
        const url = api_work_click_up + '/Update-by-key';
        return this.http.post<QueryResultsModel>(url, item, {
            headers: httpHeaders,
        });
    }
    ...
    ExportExcel(params): Observable<any> {
        ...
        return this.http.get(api_work_click_up + '/ExportExcel', { // API_work
            headers: httpHeaders,
            params: httpparams,
            // responseType: 'blob',
            // observe: 'response'
