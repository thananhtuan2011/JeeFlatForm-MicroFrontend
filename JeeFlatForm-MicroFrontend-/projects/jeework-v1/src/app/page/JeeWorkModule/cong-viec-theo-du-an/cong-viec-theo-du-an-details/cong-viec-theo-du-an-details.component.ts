...
@Component({
    selector: 'app-cong-viec-theo-du-an-details',
    templateUrl: './cong-viec-theo-du-an-details.component.html',
    styleUrls: ["./cong-viec-theo-du-an-details.component.scss"],
})
export class CongViecTheoDuAnDetaisComponent implements OnInit {
    ...
    ngOnInit(): void {
        ...
        this.activatedRoute.params.subscribe(params => {
            ...
            this.DataID_Project = +params.id_project;
            this.ShowMain = false;
            this.getParamType();
        ...
        const sb1 = this.jeeWorkV1Store.updateEvent$.subscribe(res => {
            if (res) {
                this.LoadData();
                this.LoadDetails();
            }
    ...
    getParamType() {
        ...
        if (url.includes('?')) {
            ...
            if (httpParams.get('Parent') == "1") {
                this.LoadDataParent();
                this.LoadDetails();
            }
            if (httpParams.get('Parent') == "2") {//NVLQ
                this.LoadDataNVLQ(httpParams.get('DataOldID'));
                this.LoadDetails();
            }
            if (httpParams.get('Parent') == "3") {//NVCD
                this.LoadDataNVCD(httpParams.get('DataOldID'));
                this.LoadDetails();
            }
        } else {
            this.chinhsuamota = false;//Gán lại giá trị mặc định
            this.LoadData();
            this.LoadDetails();
        }
    }
    ...
    ExportEcel(val) {
        ...
        for (let i = 0; i < val.Childs.length; i++) {
            if (val.Childs.length > 1 && i > 0) {
                child += ',' + val.Childs[i].id_row;
            }
            else {
                child += '' + val.Childs[i].id_row;
            }
        ...
        this.filter.id_project_team = val.id_project_team
        ...
        this.workService.ExportExcel(queryParams).subscribe(response => {
    ...
    getHeightMain(): any {
        let tmp_height = window.innerHeight - 10;
    ...
    LoadDetails() {
        ...
        this.weworkService.ListStatusDynamicNew(this.DataID_Project, this.DataID).subscribe((res) => {
            if (res && res.status === 1) {
    ...
    viewParent(item) {
        ...
        this.chinhsuamota = false;
        this.router.navigateByUrl(environment.HOST_ALINK + `/CongViecTheoDuAn/Details/${this.DataID_Project}/${this.DataID}?Parent=1`);//Dùng cho DPS
    }
    //===================Xem chi tiết công việc cha=======================
    updateReadItem() {
        let _item = {
    ...
    viewNVLQ(item) {
        ...
        this.DataID_Project = +item.id_project_team;
        this.chinhsuamota = false;
        this.router.navigateByUrl(environment.HOST_ALINK + `/CongViecTheoDuAn/Details/${this.DataID_Project}/${this.DataID}?Parent=2&&DataOldID=${this.DataOldID}`);//Dùng cho GOV
    }

    LoadDataNVLQ(DataOldID) {
        this.congViecTheoDuAnService.WorkDetailGOV(this.DataID, true, DataOldID, 0).subscribe((res) => {
            if (res && res.status == 1) {
                this.item = res.data;
                this.isclosed = this.item.closed;
                let repcale = this.item.description.replaceAll('color: rgb(0, 0, 0)', '');
    ...
    viewNVCD(item) {
        ...
        this.DataID_Project = item.id_project_team;
        this.chinhsuamota = false;
        this.router.navigateByUrl(environment.HOST_ALINK + `/CongViecTheoDuAn/Details/${this.DataID_Project}/${this.DataID}?Parent=3&&DataOldID=${this.DataOldID}`);//Dùng cho GOV
    }

    LoadDataNVCD(DataOldID) {
        this.congViecTheoDuAnService.WorkDetailGOV(DataOldID, true, this.DataID, 1).subscribe((res) => {
            if (res && res.status == 1) {
                this.item = res.data;
                this.isclosed = this.item.closed;
                let repcale = this.item.description.replaceAll('color: rgb(0, 0, 0)', '');
