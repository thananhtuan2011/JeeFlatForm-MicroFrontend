...
import { BaoCaoCongViecModel } from "../../../models/jee-work.model";
...
export class BaoCaoCongViecComponent implements OnInit {
  ...
  getdata(){
    ...
    this.BaoCaoService.getListTaskReport(queryParams).subscribe((res) => {
      ...
      if (res && res.status == 1) {
        ...
        console.log("data zzzzzzzzzzz",this.listData);
      }
    });
  }
  convert(str) {
  ...
  ExportExcel() {
    ...
    this.BaoCaoService.ExportExcelTaskByDepartment(list).subscribe((res) => {
