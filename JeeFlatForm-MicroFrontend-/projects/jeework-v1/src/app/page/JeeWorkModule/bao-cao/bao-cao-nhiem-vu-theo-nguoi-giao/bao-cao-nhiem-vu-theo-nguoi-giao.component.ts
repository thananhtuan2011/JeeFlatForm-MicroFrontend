...
export class BaoCaoNhiemVuTheoNguoiGiaoComponent implements OnInit {
  ...
  ExportExcel() {
    this.BaoCaoService.ExcelBaoCaoNVTNG(this.dt).subscribe((response) => {
  ...
  view(type, item) {
    ...
    this.router.navigateByUrl(environment.HOST_ALINK + `/CongViecTheoDuAn?IDDrop=6&&IDep=${this.id_department}&&IDUser=${this.User}&&ID=${type}&&IDPr=${item.id_row}`);//
  }
}
