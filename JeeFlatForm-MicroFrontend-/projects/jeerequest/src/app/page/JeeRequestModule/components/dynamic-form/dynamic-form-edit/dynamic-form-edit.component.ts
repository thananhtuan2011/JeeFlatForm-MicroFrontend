...
export class DynamicFormEditComponent implements OnInit {
	...
	createViewForm() {
		...
		this.formControls.addControl(
			"tenYeuCau",
			new FormControl(""+ this.TenYeuCau)
		);
		debugger
		for (var i = 0; i < this.controls.length; i++) {
			let control = this.controls[i];
	...
	Create(_item: any) {
		this.dynamicFormService.ChinhSuaDataFormDong(_item).subscribe((res) => {
			if (res && res.status === 1) {
				this.dynamicFormService.sendClickEvent();
