...
const JEEMEETING_API = environment.HOST_JEEMEETING_API + '/api';
...
export class JeeCommentService {
  ...
  public _countComment$ = new BehaviorSubject<any>(null);
  ...
  public GetUserReaction(item: any): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_JEECOMMENT_URL + '/comments' + `/GetUserReaction`;
  ...
  public getUriAvatar(username?: string): string {
    ...
    if (avatar && avatar.AvartarImgURL != '') {
      return avatar.AvartarImgURL;
  ...
  public postCommentModel(model: PostCommentModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_JEECOMMENT_URL + `/comments/postCommentv2`;
  ...
  public updateCommentModel(model: PostCommentModel): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_JEECOMMENT_URL + `/comments/editCommentv2`;
    return this.http.post<any>(url, model, {
      headers: httpHeaders,
    });
  }
  ...
  public deleteComment(topiccommentid: string, commentid: string): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_JEECOMMENT_URL + `/comments/deletev2/${topiccommentid}/${commentid}`;
  ...
  public deleteReplyComment(topiccommentid: string, commentid: string, replycommentid: string): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = API_JEECOMMENT_URL + `/comments/deletev2/${topiccommentid}/${commentid}/${replycommentid}`;
    return this.http.delete<any>(url, {
      headers: httpHeaders,
    });
  }
  ...
  LuuLogcomment(model): Observable<any> {
    const httpHeaders = this.httpUtils.getHTTPHeaders();
    const url = environment.HOST_JEEWORK_API_2023 + `/api/comment/luu-log-comment`;
    return this.http.post<any>(url, model, {
      headers: httpHeaders,
    });
  }
