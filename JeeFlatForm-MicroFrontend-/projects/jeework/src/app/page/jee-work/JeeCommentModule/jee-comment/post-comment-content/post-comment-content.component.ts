...
export class JeeCommentPostContentComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private _services_signlaR: JeeCommentSignalrService,
      ...
      if (!res) {
        ...
        if (this.replyCommentID) {
          this.service
            ...
            .subscribe(res => {
              this._services_signlaR._showChange$.next(res);
            });
        } else {
          this.service
            ...
            .subscribe(res => {
              this._services_signlaR._showChange$.next(res);
              const objSave: any = {};
              objSave.id_topic = this.commentID;
              objSave.comment = "";
              objSave.id_parent = 0;
              objSave.object_type = 0;
              objSave.object_id_new = this.objectID;
              objSave.listChoseTagGroup = [];
  ...
  Luulogcomment(model) {
    this.service.LuuLogcomment(model).subscribe(() => {
      var _delete: any = "delete";
      this.isEditEvent.emit(_delete);
    });
  }
  ...
  isShowEdit(username: string): boolean {
    let mainUsername = this.service.getAuthFromLocalStorage()['user']['username'];
  ...
  items: GalleryItem[] = [];
  loadlightboxImage(LstImagePanel) {
    this.items = LstImagePanel.map((item) => {
      return {
        type: 'imageViewer',
        data: {
          src: item,
