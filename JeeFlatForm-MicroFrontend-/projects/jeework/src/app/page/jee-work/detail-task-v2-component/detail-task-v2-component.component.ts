...
import { HistoryChangeAssignee } from '../MenuWork/Histoty-change-assignee/history-change-assignee.component';
...
@Component({
  selector: 'app-detail-task-v2-component',
  templateUrl: './detail-task-v2-component.component.html',
  styleUrls: ['./detail-task-v2-component.component.scss'],
})
export class DetailTaskV2ComponentComponent implements OnInit {
  ...
  sttType: any;
  linkApp: any;
  isSOffice: boolean = false;
  constructor(
    @Optional() public dialogRef: MatDialogRef<DetailTaskV2ComponentComponent>,
  ...
  ngOnInit(): void {
    ...
    if (this.linkApp == environment.Key_Soffice) this.isSOffice = true;
  ...
  openModal(
    ...
    this.dataDetailTask.deadline = this.DMYtoMDYHm(this.dataDetailTask.deadline);
  ...
  HistoryChangeAssignee() {
    const dialogRef = this.dialog.open(HistoryChangeAssignee, {
      data: { _id: this.id_task },
      panelClass: ['sky-padding-0', 'width60'],
      disableClose: true,
    });
    ...
    if (key == 'nguoitheodoi') {
      ...
      if (!this.CheckRule('assign')) {
        ...
        this.openModalChuyenXuLy(
          ...
          'Message Test',
          event,
          this._data.nguoiduocgiao.id_nv,
        );
  ...
  CheckRule(key)//key là langkey của rule, được cung cấp từ db
  {
    switch (key) {
      ...
      case 'urges': {
        if (this._data.Rule.indexOf('15') !== -1) {
          return true; // Quyền thay đổi cập nhật đôn đốc
        }
  ...
  Dinhkemketqua(val) {
    ...
    const dialogRef = this.dialog.open(EditorJeeWorkComponent, {
      data: {
        Doccument_Result: this.Doccument_Result,
        id_task: this.id_task,
        value: val.id_row,
        isProgress: this.CheckRule('Attachments_result'), type: this.sttType, isDefault: this._data.status.isDefault,
        ketqua: 'status',
      },
      width: '55%',
      ...
      .subscribe((res) => {
        if (res) {
          if (res.status == 1) {
            ...
            if (data_task.Doccument_Result.length > 0) {
              this.Doccument_Result = data_task.Doccument_Result;
            }
            this._data.department = data_task.department;
            this._data.Tags = data_task.Tags;

            this._data.Followers = data_task.Followers;
            this._data.isRefuseWork = data_task.isRefuseWork;
            ...
            if (data_task.deadline != null && data_task.deadline != "") {
              ...
              const [day, month, year] = t[1].split('/');
              const date = new Date(+year, +month - 1, +day);
              this._data.deadline = date;
              // this._data.deadline= this._FormatTimeService.format_convertDate(data_task.deadline,3,false);
              // let t = data_task.deadline.split(' ');
              // let d =
            ...
            this._data.Attachments_Result_Gov = data_task.Attachments_Result_Gov;
            this._data.Doc_Urges = data_task.Doc_Urges;
  ...
  save_file_Direct(evt: any, type: string) {
    ...
    else//==11, thêm tài liệu cho văn bản kết quả
    {
      if (!this.CheckRule('Attachments_result')) {
        this.layoutUtilsService.showError(
          this.translate.instant('work.bankhongcoquyenthuchienthaotacnay')
  ...
  //==============================Bổ sung button từ chối==================================
  RefuseWork() {
    const dialogRef = this.dialog.open(EditorJeeWorkComponent, {
      data: { statusname: 'Từ chối nhiệm vụ' },
      width: '55%',
      maxHeight: '60%',
      panelClass: ['editor-v2']
    });
    ...
    dialogRef.afterClosed().subscribe((res) => {
      ...
      let item = new UpdateModel();
      item.ReasonRefuse = res._result
      item.id_row = this.id_task;
      this.update('refuse_work', item);
    });
  }

  isRefuseWork() {
    if (this._data && this._data.isRefuseWork && this._data.status.type != 4) return true;
    return false;
  }
  ...
  getDisplay(stt) {
    if (this._data) {
      if (this._data.status && this._data.status.id_row == stt.id_row) {
        return 'none'
      }
      return 'content';
    }
  ...
  //===================Bổ sung xem luồng trực tiếp===============
  XemLuongNhiemVuTrucTiep() {
    const dialogRef = this.dialog.open(CongViecTheoDuAnChartComponent, {
      data: { _id: this.id_task, _type: 3 },
      panelClass: ['sky-padding-0', 'width70'],
      disableClose: true,
    });
    dialogRef.afterClosed().subscribe((res) => {
  ...
  //===================Bổ sung xem tiến độ nhiệm vụ ở chỉnh sửa kết quả nhiệm vụ===============
  LoadProgress() {
  ...
  //==============Thay đổi cách check quyền chỉnh sửa nhiệm vụ===========================
  ...
  //=======Bổ sung chức năng cập nhật đôn đốc========================================================
  UpdateDonDoc() {
    const dialogRef = this.dialog.open(EditorUrgesComponent, {
      data: {
        id_task: this.id_task,
        isUrges: this.CheckRule('urges'), 
        type: this._data.status.type, isDefault: this._data.status.isDefault,
      },
      width: '65%',
      panelClass: ['editorstt-v2']
    });
...
export class DetailTaskModel {
  ...
  empty() {
    ...
    this.createddate = '';
    this.status = { color: '', statusname: '', id_row: '', type: 0, isDefault: false };
    this.nguoiduocgiao = {
      hoten: '',
      tenchucdanh: '',
      username: '',
...
export class UpdateModel {
  ...
  SoVB_Result: string;
  NgayVB_Result: string;
  NguoikyVB_Result: string;
  TrichyeuVB_Result: string;
  id_status:string;
  emty() {
    this.values = [];
    this.id_row = '';
    ...
    this.SoVB_Result = '';
    this.NgayVB_Result = '';
    this.NguoikyVB_Result = '';
    this.TrichyeuVB_Result = '';
