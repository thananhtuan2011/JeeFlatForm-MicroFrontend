  ...
  loadTags() {
    this.DanhMucService.ListTagByProject(
      ...
      if (res.status == 1) {
        ...
        if(this.idmenu!= undefined && this.idmenu=="7" && this.demSL==0){
          this.demSL++;
          let index = this.list_TagFull.findIndex(t=>t.rowid ==this.idTag);
          if(index>=0){
            this.first_Tag=this.list_TagFull[index];
            this.select(this.first_Tag);
          }
        }
