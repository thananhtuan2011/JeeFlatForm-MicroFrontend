...
import { NhiemVuDuocGiaoModel } from "../Model/jee-work.model";
...
@Component({
  selector: "app-bao-cao-tong-gio-lam",
  templateUrl: "./bao-cao-tong-gio-lam.component.html",
  styleUrls: ["./bao-cao-tong-gio-lam.component.scss"],
})
export class BaoCaoTongGioLamComponent implements OnInit {
  ...
  getdata() {
    ...
    this.BaoCaoService.getListSumWorkTime(queryParams).subscribe((res) => {
      if (res && res.status == 1) {
  ...
  view(type, item) {
    ...
    this.router.navigateByUrl(`Work/CongViecTheoDuAn?IDDrop=6&&IDep=${this.id_department}&&IDUser=${item.id_nguoigiao}&&ID=${type}&&IDPr=${this.id_project_team}`);//Thiên mở 6/1/23
