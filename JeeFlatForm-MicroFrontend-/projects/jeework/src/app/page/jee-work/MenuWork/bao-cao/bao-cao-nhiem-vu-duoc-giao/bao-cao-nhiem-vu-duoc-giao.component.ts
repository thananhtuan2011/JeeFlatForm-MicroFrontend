...
@Component({
  selector: "app-bao-cao-nhiem-vu-duoc-giao",
  templateUrl: "./bao-cao-nhiem-vu-duoc-giao.component.html",
  styleUrls: ["./bao-cao-nhiem-vu-duoc-giao.component.scss"],
})
export class BaoCaoNhiemVuDuocGiaoComponent implements OnInit {
  ...
  getdata() {
    ...
    this.BaoCaoService.getListReportTaskAssign(queryParams).subscribe((res) => {
      if (res && res.status == 1) {
  ...
  ExportExcel() {
    ...
    this.BaoCaoService.ExcelReportListTaskAssign(queryParams,this.dt).subscribe((response) => {
  ...
  view2(type, item) {
    this.filterDay.startDate=moment(this.filterDay.startDate, ['DD/MM/YYYY']).format('DD/MM/YYYY');
    this.filterDay.endDate=moment(this.filterDay.endDate, ['DD/MM/YYYY']).format('DD/MM/YYYY');
    ...
    this.router.navigate(['', { outlets: { auxName: 'auxWork/List/' + this.chon + '/' + this.Departmemnt + '/' + this.id_project_team + '/' + type + '/' + item.id_nguoigiao }, }],{state:{filterDay:this.filterDay}});

    //this.router.navigate(['', { outlets: { auxName: 'auxWork/List/12/' + this.id_project_team + '/' + type + '/' + item.id_nguoigiao }, }]);

    //this.router.navigate(['', { outlets: { auxName: 'auxWork/List/10/' + '513' + '/' + type }, }]);
  }
  ...
  view(type, item) {
    ...
    // this.router.navigateByUrl(`Work/CongViecTheoDuAn?IDDrop=6&&IDep=${this.id_department}&&IDUser=${item.id_nguoigiao}&&ID=${type}`);// Thiên đóng 6/1/23
  }
  ...
  setItemLocalStorate(type, item) {
    ...
    data.tien_do = type;
    data.id_user_giao = item.id_nguoigiao
    ...
    // localStorage.setItem('loaicv', type);
    // localStorage.setItem('id_project_team', type);
