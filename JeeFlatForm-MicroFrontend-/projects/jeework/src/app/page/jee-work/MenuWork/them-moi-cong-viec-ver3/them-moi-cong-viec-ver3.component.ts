...
@Component({
  selector: 'app-them-moi-cong-viec-ver3',
  templateUrl: './them-moi-cong-viec-ver3.component.html',
  styleUrls: ['./them-moi-cong-viec-ver3.component.scss'],
})
export class ThemMoiCongViecVer3Component implements OnInit {
  idmeeting: number;
  modalTitle: string;
  modalMessage: string;
  ...
  constructor(
    public dialogRef: MatDialogRef<ThemMoiCongViecVer3Component>,
  ...
  ngOnInit(): void {
    ...
    // this.LoadDataDuAn(); //Thiên đóng 1/11/2023
  ...
  onSubmit(withBack: boolean = false, typeSave = 1) {
    let isFlag = true;
    if ((this.requireAss && this.IsAssignee == 1) && !this.chonUser) {
      this.layoutUtilsService.showActionNotification(
