...
export class BaoCaoNhiemVuDuocTaoComponent implements OnInit {
  ...
  ExportExcel() {
    ...
    this.BaoCaoService.ExcelReportListTaskCreated(queryParams,this.dt).subscribe((response) => {
  ...
  view2(type, item) {
    ...
    this.router.navigate(['', { outlets: { auxName: 'auxWork/List/'+this.chon+'/' + this.id_department + '/' + this.id_project_team + '/' + type + '/' + item.id_nguoitao }, }],{state:{filterDay:this.filterDay}});
  }
