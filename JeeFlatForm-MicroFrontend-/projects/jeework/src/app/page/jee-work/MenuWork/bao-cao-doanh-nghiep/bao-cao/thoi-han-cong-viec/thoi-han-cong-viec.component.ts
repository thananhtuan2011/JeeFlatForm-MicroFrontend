...
import { ThoiHanCongViecService } from "../../Services/thoi-han-cong-viec.service";
...
@Component({
  selector: "bao-cao-thoi-han-cong-viec",
  templateUrl: "./thoi-han-cong-viec.component.html",
})
export class ThoiHanCongViecBaoCaoComponent
  implements
  OnInit,
  OnDestroy {
  ...
  btnFilterDK_ThoiHanCV: Subscription;
  btnDeleteFilterDK_ThoiHanCV: Subscription;
  btnThietlap_ThoiHanCV: Subscription;

  @Input()
  btnFilterEventThoiHanCV: EventEmitter<any> = new EventEmitter<any>();
  filterDSThoiHanCV: EventEmitter<any> = new EventEmitter<any>();
  btnLoaiBieuDo: EventEmitter<any> = new EventEmitter<any>();
  filterDieuKienLoc_ThoiHanCV: EventEmitter<any> = new EventEmitter<any>();

  btnDeleteFilterEventCondition_ThoiHanCV: EventEmitter<any> = new EventEmitter<any>();
  btnThietlapFilter_ThoiHanCV: EventEmitter<any> = new EventEmitter<any>();

  btnFilterEventCondition_ThoiHanCV: EventEmitter<any> = new EventEmitter<any>();
  ...
  constructor(
    ...
    public _ThoiHanCongViecService: ThoiHanCongViecService,
  ...
  ngOnInit() {
    ...
    this.btnFilterSub = this.btnFilterEventThoiHanCV.subscribe((res) => {
      if (res != this.Team) {
        this.Team = res;
        setTimeout(() => {
        ...
        this.filterDSThoiHanCV.emit(object);
      }
    });
    ...
    this.btnFilterDK_ThoiHanCV = this.btnFilterEventCondition_ThoiHanCV.subscribe((res) => {
      if (res != this.idDK) {
        this.idDK = res.RowID;
        ...
        this.filterDieuKienLoc_ThoiHanCV.emit(object);
      }
    });
    this.btnDeleteFilterDK_ThoiHanCV = this.btnDeleteFilterEventCondition_ThoiHanCV.subscribe((res) => {
      this.delete_dk(res);

    });
    this.btnThietlap_ThoiHanCV = this.btnThietlapFilter_ThoiHanCV.subscribe((res) => {
      this.load_đk();
  ...
  load_đk() {
    this._BaoCaoDoanhNghiepService.Get_listCustomWidgetByUser(9).subscribe(res => {
      if (res && res.status == 1) {
  ...
  delete_dk(id) {
    this._BaoCaoDoanhNghiepService.Delete_Custom_Widget(id).subscribe(res => {
      if (res && res.status == 1) {
        this.load_đk();
      }
    })
  ...
  pieChart(query) {
    this._ThoiHanCongViecService.loadList(query).subscribe(res => {
      if (res && res.status == 1) {
        if (res && res.data["ThoiHanCongViec"]) {
          this.pieChartLabel = res.data["ThoiHanCongViec"];
          this.changeDetectorRefs.detectChanges();
        }
        ...
        if (res.data["DataThoiHanCongViec"]) {
          this.pieChartData = res.data["DataThoiHanCongViec"];
          this.changeDetectorRefs.detectChanges();
        }
      }
    });
