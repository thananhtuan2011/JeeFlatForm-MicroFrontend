...
export class FormatTimeService {
    ...
    //Value: Thời gian (2021-06-08 14:33:12Z), Type: 1-Thời gian rút gọn, 2-Thời gian đầy đủ,3-Thời gian rút gọn dd/mm/yy, IsTime: true có hiện thị giờ, false không hiển thị giờ
    format_convertDate(Value: any, Type: any, IsTime: boolean) {
        ...
        // }
        //Giá trị đầu vào
        let date_value = new Date(Value+'Z');
        ...
        if (Type == 1) {
            ...
            date_return = this.case_time_only_date(days, date_value, date_now, langcode)
        }
        return date_return;
    }
    ...
    case_priority_1_istime(daynum: any, date_value, date_now, langcode: string) {
        ...
        if (daynum > 1) {//Sau hiện tại hơn 1 ngày
            ...
            if (langcode == "vi") {
                date = this.datePipe.transform(date_value, 'HH:mm');
            } else {
    ...
    f_date_thu(value: any): any {
     ...
     //=======================Thời gian chỉ hiện thị ngắn gọn dd/mm/yy==================================
     case_time_only_date(daynum: any, date_value, date_now, langcode: string) {
        let date = '';
        if (langcode == "vi") {
            date = ("0" + (date_value.getDate())).slice(-2) + "/" + ("0" + (date_value.getMonth() + 1)).slice(-2) + "/" + `${date_value.getFullYear().toString().slice(2)}`;
        } else {
            date = this.datePipe.transform(date_value, 'MMM d, yyyy');
        }
        return date;
