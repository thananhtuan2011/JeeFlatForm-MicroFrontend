...
import { DanhMucChiNhanhEditDialogComponent } from '../danh-muc-chi-nhanh-edit-dialog/danh-muc-chi-nhanh-edit-dialog.component';


@Component({
  selector: 'app-danh-muc-chi-nhanh-list',
  templateUrl: './danh-muc-chi-nhanh-list.component.html',
  styleUrls: ["./danh-muc-chi-nhanh-list.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DanhMucChiNhanhListComponent implements OnInit {
  ...
  flag: boolean;
  constructor(
    public _DanhMucChiNhanhService: DanhMucChiNhanhService,
  ...
  TroLai() {
    this.router.navigate([`/WizardSale/Introduce`]);
  }
  ...
  Add() {
    const chucvuModels = new DM_CNModel();
    chucvuModels.clear(); // Set all defaults fields
    this.Edit(chucvuModels);
  }

  Edit(_item: DM_CNModel) {
    let saveMessageTranslateParam = '';
    saveMessageTranslateParam += _item.IDChiNhanh > 0 ? 'JeeHR.capnhatthanhcong' : 'JeeHR.themthanhcong';
    const _saveMessage = this.translate.instant(saveMessageTranslateParam);
    const _messageType = _item.IDChiNhanh > 0 ? MessageType.Update : MessageType.Create;
    const dialogRef = this.dialog.open(DanhMucChiNhanhEditDialogComponent, { data: { _item }, panelClass: ['hr-padding-0', 'width600'] });
  ...
  Delete(_item: DM_CNModel) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      this._DanhMucChiNhanhService.Delete(_item.IDChiNhanh).subscribe(res => {
        if (res && res.status === 1) {
  ...
  LoadData() {
    ...
    this._DanhMucChiNhanhService.findData(queryParams).subscribe(
      (res) => {
  ...
  LoadDataLazy() {
    ...
    queryParams.pageSize = this.record;
    queryParams.more = this.more;
    this._DanhMucChiNhanhService.findData(queryParams).subscribe((res) => {
      this.layoutUtilsService.OffWaitingDiv();
      if (res && res.status == 1) {
