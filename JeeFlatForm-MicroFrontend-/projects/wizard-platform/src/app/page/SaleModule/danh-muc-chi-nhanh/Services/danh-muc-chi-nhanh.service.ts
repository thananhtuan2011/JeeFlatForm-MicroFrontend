...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/agency';

@Injectable()
export class DanhMucChiNhanhService {
	...
	getDM_CNById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders });
	}

	Update(item: DM_CNModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
