...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/warehouse';

@Injectable()
export class DanhMucKhoService {
	...
	getDM_KhoById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders });
	}

	Update(item: DM_KhoModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
