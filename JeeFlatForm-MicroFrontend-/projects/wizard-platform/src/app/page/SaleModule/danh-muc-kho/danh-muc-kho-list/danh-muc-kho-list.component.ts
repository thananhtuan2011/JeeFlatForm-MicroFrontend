...
import { DanhMucKhoEditDialogComponent } from '../danh-muc-kho-edit-dialog/danh-muc-kho-edit-dialog.component';


@Component({
  selector: 'app-danh-muc-kho-list',
  templateUrl: './danh-muc-kho-list.component.html',
  styleUrls: ["./danh-muc-kho-list.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DanhMucKhoListComponent implements OnInit {
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = [
    'STT',
    'TenChiNhanh', 'MaKho', 'TenKho', 'TrangThai', 'DienThoai', 'DiaChi', 'actions'
  ];
  sortOrder: string;
  sortField: string;
  page: number;
  ...
  flag: boolean;
  constructor(
    public _DanhMucKhoService: DanhMucKhoService,
  ...
  TroLai() {
    this.router.navigate([`/WizardSale/Store`]);
  }
  ...
  Add() {
    const chucvuModels = new DM_KhoModel();
    chucvuModels.clear(); // Set all defaults fields
    this.Edit(chucvuModels);
  }

  Edit(_item: DM_KhoModel) {
    let saveMessageTranslateParam = '';
    saveMessageTranslateParam += _item.IDKho > 0 ? 'JeeHR.capnhatthanhcong' : 'JeeHR.themthanhcong';
    const _saveMessage = this.translate.instant(saveMessageTranslateParam);
    const _messageType = _item.IDKho > 0 ? MessageType.Update : MessageType.Create;
    const dialogRef = this.dialog.open(DanhMucKhoEditDialogComponent, { data: { _item }, panelClass: ['hr-padding-0', 'width600'] });
  ...
  Delete(_item: DM_KhoModel) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      this._DanhMucKhoService.Delete(_item.IDKho).subscribe(res => {
        if (res && res.status === 1) {
