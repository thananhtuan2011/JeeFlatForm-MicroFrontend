...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/customer-group';

@Injectable()
export class DanhMucNhomKhachService {
	...
	getDM_Loai_KHById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders });
	}

	Update(item: DM_Loai_KHModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
