...
import { DanhMucNhomKhachEditDialogComponent } from '../danh-muc-nhom-khach-edit-dialog/danh-muc-nhom-khach-edit-dialog.component';


@Component({
  selector: 'app-danh-muc-nhom-khach-list',
  templateUrl: './danh-muc-nhom-khach-list.component.html',
  styleUrls: ["./danh-muc-nhom-khach-list.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class DanhMucNhomKhachListComponent implements OnInit {
  dataSource = new MatTableDataSource<any>([]);
  displayedColumns: string[] = [
    'STT',
    'MaNhomKH', 'TenNhomKH', 'GhiChu', 'actions',
  ];
  sortOrder: string;
  sortField: string;
  page: number;
  ...
  flag: boolean;
  constructor(
    public _DanhMucNhomKhachService: DanhMucNhomKhachService,
  ...
  TiepTuc() {
    this.router.navigate([`/WizardSale/Customer`]);
  }

  Add() {
    const chucvuModels = new DM_Loai_KHModel();
    chucvuModels.clear(); // Set all defaults fields
    this.Edit(chucvuModels);
  }

  Edit(_item: DM_Loai_KHModel) {
    let saveMessageTranslateParam = '';
    saveMessageTranslateParam += _item.IDNhomKH > 0 ? 'JeeHR.capnhatthanhcong' : 'JeeHR.themthanhcong';
    const _saveMessage = this.translate.instant(saveMessageTranslateParam);
    const _messageType = _item.IDNhomKH > 0 ? MessageType.Update : MessageType.Create;
    const dialogRef = this.dialog.open(DanhMucNhomKhachEditDialogComponent, { data: { _item }, panelClass: ['hr-padding-0', 'width600'] });
  ...
  Delete(_item: DM_Loai_KHModel) {
    ...
    dialogRef.afterClosed().subscribe(res => {
      ...
      this._DanhMucNhomKhachService.Delete(_item.IDNhomKH).subscribe(res => {
        if (res && res.status === 1) {
  ...
  LoadData() {
    ...
    this._DanhMucNhomKhachService.findData(queryParams).subscribe(
      (res) => {
  ...
  LoadDataLazy() {
    ...
    queryParams.pageSize = this.record;
    queryParams.more = this.more;
    this._DanhMucNhomKhachService.findData(queryParams).subscribe((res) => {
      this.layoutUtilsService.OffWaitingDiv();
      if (res && res.status == 1) {
