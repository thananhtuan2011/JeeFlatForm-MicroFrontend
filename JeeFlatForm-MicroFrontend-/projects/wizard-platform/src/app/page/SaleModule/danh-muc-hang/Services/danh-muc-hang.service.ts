...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/goods';

@Injectable()
export class DanhMucHangService {
	...
	getDM_MatHangById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders });
	}

	Update(item: DM_MatHangModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	getThueVATMacDinh(): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/GetVATDefault`, { headers: httpHeaders });
	}
}
