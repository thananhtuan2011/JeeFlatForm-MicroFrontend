...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/customer-sale';
const HOST_JEEACCOUNT_API = environment.HOST_JEEACCOUNT_API ;

@Injectable()
export class DanhMucKhachService {
	...
	getDM_KHById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders });
	}

	Update(item: DM_KHModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
	...
	UpdateInitStatusApp(): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		const url = HOST_JEEACCOUNT_API + `/api/permissionmanagement/UpdateInitStatusApp/38/2`;
