...
const API_PRODUCTS_URL = environment.HOST_JEESALE_API + '/api/type-goods';

@Injectable()
export class DanhMucNhomHangService {
	...
	getDM_LoaiMatHangById(itemId: any): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
		return this.http.get<any>(API_PRODUCTS_URL + `/${itemId}`, { headers: httpHeaders, });
	}

	Update(item: DM_LoaiMatHangModel): Observable<any> {
		const httpHeaders = this.httpUtils.getHTTPHeaders();
