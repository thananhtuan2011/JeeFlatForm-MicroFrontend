...
@Component({
  selector: 'app-cau-hinh-cham-cong-list',
  templateUrl: './cau-hinh-cham-cong-list.component.html',
  styleUrls: ["./cau-hinh-cham-cong-list.component.scss"],
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CauHinhChamCongListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this._DanhMucChungService.getStrConfig(1,"hr_wifi").subscribe(res => {
			if (res && res.status == 1 && res.data.length > 0) {
				//Giới thiệu
				this._DanhMucChungService.textHR8_1 = res.data[0] ? res.data[0].Mota : "";
			}
