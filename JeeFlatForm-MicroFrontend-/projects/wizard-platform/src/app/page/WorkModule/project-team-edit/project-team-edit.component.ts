...
export class ProjectTeamEditComponent implements OnInit {
	...
	ngOnInit() {
		...
		this._WorkwizardChungService.lite_department().subscribe(res => {
		...
		this._WorkwizardChungService.get_template(queryParams).subscribe(res => {
			if (res && res.status === 1) {
		...
		if (this.item.id_row > 0) {
			...
			this._WorkwizardChungService.Project_Detail(this.item.id_row).subscribe(res => {
				if (res && res.status == 1) {
					this.item = res.data;
	...
	loadListRole(queryParams) {
		this._WorkwizardChungService.findRoleListProject(queryParams).subscribe(res => {
			if (res && res.status === 1) {
	...
	loadaccount() {
		...
		this._WorkwizardChungService.list_account_customer().subscribe(res => {
			this.disabledBtn = false;
			if (res && res.status === 1) {
	...
	loadGroupStatus() {
		...
		this._WorkwizardChungService.ListGroupStatus(queryParams1).subscribe(res => {
			if (res && res.status == 1) {
	...
	protected filterProject() {
		...
		this.filtereproject.next(
			this.listdepartment.filter(proj => this._WorkwizardChungService.toNonAccentVietnamese(proj.Title).toLowerCase().indexOf(this._WorkwizardChungService.toNonAccentVietnamese(search)) > -1)
		);
	}
	...
	/** UI */
	getTitle(): string {
		let result = this.translate.instant('JeeWork.themmoi');
		if (!this.item || !this.item.id_row) {
			return result;
		}
		result = this.translate.instant('JeeWork.chinhsua');
		return result;
	}
	...
	Update(_item: ProjectTeamModelV2, withBack: boolean) {
		...
		this._WorkwizardChungService.UpdateProjectTeam2(_item).subscribe(res => {
			this.disabledBtn = false;
			...
			if (res && res.status === 1) {
				...
				const _messageType = this.translate.instant('JeeWork.capnhatthanhcong');
			}
			else {
	...
	BindUserDepartment(id) {
		...
		this._WorkwizardChungService.list_account_department(id).subscribe((res) => {
			if (res) {
				if (res.status == 1) {
	...
	isKtra:boolean=false;
	Create(_item: ProjectTeamModelV2, withBack: boolean) {
		this.loadingAfterSubmit = true;
		this.disabledBtn = true;
		// this.layoutUtilsService.showWaitingDiv();
		this._WorkwizardChungService.InsertProjectTeam2(_item).subscribe(res => {
			this.disabledBtn = false;
			// this.layoutUtilsService.OffWaitingDiv();
			if (res && res.status == 1) {
				if (withBack == true) {
				...
				const _messageType = this.translate.instant('JeeWork.themthanhcong');
	...
	close() {
		...
		const _deleteMessage = this.translate.instant('JeeWork.thaydoithanhcong');
		if (this.isChangeData()) {
			...
			dialogRef.afterClosed().subscribe(res => {
				...
				this.dialogRef.close(this.isKtra);
			});
		} else this.dialogRef.close(this.isKtra);
	}
