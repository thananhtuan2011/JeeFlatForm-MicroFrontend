...
export class FormatTimeService {
    ...
    case_priority_0_notistime(daynum: any, date_value, date_now, langcode: string) {
        ...
        if (daynum > 1) {//Sau hiện tại hơn 1 ngày
            if (date_value.getFullYear() == date_now.getFullYear()) {
                if (langcode == "vi") {
                    date = ("0" + (date_value.getDate())).slice(-2) + " tháng " + ("0" + (date_value.getMonth() + 1)).slice(-2);
                } else {
                    date = this.datePipe.transform(date_value, 'MMM d');
                }
            } else {
                if (langcode == "vi") {
                    date = ("0" + (date_value.getDate())).slice(-2) + " tháng " + ("0" + (date_value.getMonth() + 1)).slice(-2) + ", " + date_value.getFullYear();
                } else {
    ...
    f_date_thuso2(value: any): any {
        if (value == null || value == "") {
            return "";
        }
        let e = this.datePipe.transform(value, 'EEEE');
        let latest_date = this.datePipe.transform(value, 'EEEE, dd/MM/yyyy - HH:mm');
        switch (e) {
            case "Monday":
                latest_date = latest_date.replace("Monday", "Thứ 2");
                break;
