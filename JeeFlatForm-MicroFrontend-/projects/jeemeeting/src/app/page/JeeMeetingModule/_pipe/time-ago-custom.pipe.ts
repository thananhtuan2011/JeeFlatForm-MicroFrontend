...
export class TimeAgoPipeCus implements PipeTransform {
  transform(d: any): string {
    ...
    if (createdSecond >= 31556926) {
      ...
      // }
      var date1_tomorrow = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate() + 1);
      var date1_today = new Date(currentDate.getFullYear(), currentDate.getMonth(), currentDate.getDate());
      if (date1_tomorrow.getFullYear() == date.getFullYear() && date1_tomorrow.getMonth() == date.getMonth() && date1_tomorrow.getDate() == date.getDate()) {
        return this.convertHour(date) + " ngày mai"
      }else
      if (date1_today.getFullYear() == date.getFullYear() && date1_today.getMonth() == date.getMonth() && date1_today.getDate() == date.getDate()) {
        return this.convertHour(date) + " hôm nay"
      }else{
        return this.convertHour(date) + " " + this.convertDate(date)
      }
