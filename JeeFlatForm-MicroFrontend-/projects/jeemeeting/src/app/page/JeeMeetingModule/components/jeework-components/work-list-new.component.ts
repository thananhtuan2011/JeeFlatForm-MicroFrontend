...
import { CommunicateService } from "./_services/communicate.service";

import { C } from "@angular/cdk/keycodes";
import { WorkGroupEditComponent } from './work-group-edit/work-group-edit.component';
import { MenuPhanQuyenServices } from './_services/menu-phan-quyen.service';
...
export class WorkListNewComponent implements OnInit {
    ...
    this.weworkService
      ...
      .subscribe((res) => {
        if (res && res.status == 1) {
          ...
          this.ListColumns = res.data.filter(x => x.show_default_type == 1);
          // xóa title khỏi cột
  ...
  ViewDetai(item) {
    // const url = `/Work/CongViecTheoDuAn/${this.ID_Project}/${item.id_row}`;
  ...
  UpdateListDuplicate(type) {
    ...
    if (type == 1) {
      ...
      const dialogRef = this.dialog.open(DuplicateTaskNewComponent, {
        ...
        disableClose:true,
        panelClass:'no-padding',data: { getOnlyIDproject: true },
      });
      dialogRef.afterClosed().subscribe((res) => {
        if (res) {
          this.selection.selected.forEach((element) => {
 ...
 panelClass:'no-padding',
  ...
  UpdateField(item) {
    ...
    const dialogRef = this.dialog.open(AddNewFieldsComponent, {
      ...
      disableClose:true,
      panelClass:'no-padding',  width: "600px",
      data: _item,
    });
