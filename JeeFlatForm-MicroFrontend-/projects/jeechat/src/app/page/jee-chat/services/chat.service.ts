import { QueryParamsModel, QueryParamsModelNewLazy, QueryResultsModel } from './../models/pagram';
...
const API_JEEWORK_lite = "https://jeework-api.jee.vn" + "/api/wework-lite";
const API_JEEWORK_PC = 'https://work-api.jee.vn' + '/api/work/list-task-by-jeechat';
@Injectable({
  providedIn: 'platform'
})
export class ChatService {
  ...
  getHTTPHeadersTimeZone(version: any = '1.0'): HttpHeaders {
    const access_token = this.httpUtils.getToken()
  ...
  publishCreateGroup(item: any) {
    const url = this.baseUrl + `/notifi/publishCreateGroup`;
    const httpHeader = this.getHttpHeaders();
    return this.http.post<any>(url, item, { headers: httpHeader });
  }
  publishMessNotifiGhim(token: any, idGroup: number, mesage: string, idchat: number) {
    const url = this.baseUrl + `/notifi/publishMessNotifiGhim?token=${token}&idGroup=${idGroup}&mesage=${mesage}&idchat=${idchat}`;
    const httpHeader = this.getHttpHeaders();
    return this.http.get<any>(url, { headers: httpHeader });
  }
  GetGhimGroup(IdGroup: number) {
    const url = this.baseUrl + `/chat/GetGhimGroup?IdGroup=${IdGroup}`;
    const httpHeader = this.getHttpHeaders();
    return this.http.get<any>(url, { headers: httpHeader });
  }
  loadCongViecTheoDuAnForChat(queryParams: QueryParamsModel): Observable<any> {
    const httpHeaders = this.getHTTPHeadersTimeZone();
    const httpParams = this.httpUtils.getFindHTTPParams(queryParams);
    const url = API_JEEWORK_PC;
  ...
  CheckGov() {
    const url = `https://work-api.jee.vn/api/general/check-isgov`;
    const httpHeader = this.getHttpHeaders();
    return this.http.get<any>(url, { headers: httpHeader });
  }

  removeGhim(IdGroup) {
    const url = this.baseUrl + `/chat/removeGhim?IdGroup=${IdGroup}`;
    const httpHeader = this.getHttpHeaders();
    return this.http.post<any>(url, null, { headers: httpHeader });
  }
  addGhim(item) {
    const url = this.baseUrl + `/chat/addGhim`;
    const httpHeader = this.getHttpHeaders();
    return this.http.post<any>(url, item, { headers: httpHeader });
  }
  ...
  publishMessNotifiWebNoActive(IdGroup: number, mesage: string, fullname: string, avatar: string,
    isEncode: boolean, isSticker: boolean
  ) {
    const url = this.baseUrl + `/notifi/publishMessNotifiTwoUserWeb?IdGroup=${IdGroup}
    &mesage=${mesage}&fullname=${fullname}&avatar=${avatar}&isEncode=${isEncode}&isSticker=${isSticker}`;
    const httpHeader = this.getHttpHeaders();
    return this.http.get<any>(url, { headers: httpHeader });
  }
  publishMessNotifiWebNoActiveGroup(IdGroup: number, mesage: string, fullname: string, avatar: string,
    isEncode: boolean, isSticker: boolean
  ) {
    const url = this.baseUrl + `/notifi/publishMessNotifiGroupWeb?IdGroup=${IdGroup}
      &mesage=${mesage}&fullname=${fullname}&avatar=${avatar}&isEncode=${isEncode}&isSticker=${isSticker}`;
    const httpHeader = this.getHttpHeaders();
    return this.http.get<any>(url, { headers: httpHeader });
  }
  ...
  UpdateDataUnreadForActiveTab(IdGroup: number, UserId: number) {
    const url = this.baseUrl + `/chat/UpdateDataUnreadForActiveTab?IdGroup=${IdGroup}&UserID=${UserId}`
    const httpHeader = this.getHttpHeaders();
    return this.http.post<any>(url, null, { headers: httpHeader });
  }
