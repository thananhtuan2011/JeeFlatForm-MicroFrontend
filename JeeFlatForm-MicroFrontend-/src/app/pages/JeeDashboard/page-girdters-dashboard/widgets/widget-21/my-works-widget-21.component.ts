...
import { WidgetWork21Service } from "../../Services/works-widget-21.service";
...
@Component({
  selector: "my-works-widget-21",
  templateUrl: "./my-works-widget-21.component.html",
  styleUrls: ["./my-works-widget-21.component.scss"],
})
export class MyWorksWidget21Component
  implements
  OnInit,
  OnDestroy {
  ...
  @Input() cssClass: "";
  btnPriority21: EventEmitter<any> = new EventEmitter<any>();
  btnFilterSub: Subscription;
  Priority = 0;
  constructor(
    private changeDetectorRefs: ChangeDetectorRef,
    public _WidgetWork21Service: WidgetWork21Service,
  ...
  ngOnInit() {
    ...
    this.btnFilterSub = this.btnPriority21.subscribe((res) => {
      this.Priority = res;
      this.loadList();
  ...
  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height21").clientHeight;
    tmp_height = tmp_height - 120;
    return tmp_height + "px";
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height21").clientWidth;
  ...
  loadList() {
    ...
    this.isLoading = true;
    this._WidgetWork21Service.loadList(queryParams).subscribe(res => {
  ...
  loadDataList_Lazy() {
    if (!this._loading) {
      ...
      if (this.crr_page < this.total_page) {
        ...
        this.isLoading = true;
        this._WidgetWork21Service.loadList(queryParams)
          .pipe(
