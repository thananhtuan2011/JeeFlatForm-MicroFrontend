...
export class SetUpWidgetTimeComponent implements OnInit {
  ...
  Detail_custom() {
    this._WorkGeneralService.Detail_custom(this.data.id_custom).subscribe(res => {
      if (res && res.status == 1) {
  ...
  loaddieukien() {
    this._WorkGeneralService.Get_listCustom().subscribe(res => {
      if (res && res.status == 1) {
  ...
  Thongtin() {
    ...
    const dialogRef = this.dialog.open(MotanoidungSetTimeComponent, { data: { item }, width: '700px', panelClass: 'sky-padding-0' });
  ...
  Onsubmit(withBack: boolean) {
    if (this.Date_ && this.Operators && this.CustomID) {
      ...
      if (this.data.id_custom != 0) {
        this._WorkGeneralService.Update_custom_widget(item).subscribe(res => {
          if (res && res.status == 1) {
            this.dialogRef.close();
          }
        })
        ...
        this._WorkGeneralService.Insert_custom_widget(item).subscribe(res => {
          if (res && res.status == 1) {
            if (withBack) {
              this.dialogRef.close();
            }
