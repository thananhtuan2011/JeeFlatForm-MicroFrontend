...
@Component({
  selector: "widget-cong-viec-quan-trong",
  templateUrl: "./widget-cong-viec-quan-trong.component.html",
  styleUrls: ["./widget-cong-viec-quan-trong.component.scss"],
})
export class WidgetCongViecQuanTrongComponent
  implements
  OnInit,
  OnDestroy {
  ...
  btnFilterTag22: EventEmitter<any> = new EventEmitter<any>();
  btnFilterTag: Subscription;
  Tag = 0;
  filterEvent22: EventEmitter<any> = new EventEmitter<any>();
  filterTag22: EventEmitter<any> = new EventEmitter<any>();
  btnLoaiCongViec: EventEmitter<any> = new EventEmitter<any>();
  ...
  constructor(
    ...
    public _WidgetCVQTService: WidgetCongViecQuanTrongService,
  ...
  ngOnInit() {
    this.LoadData();
    this.btnFilterSub = this.btnLoaiCongViec.subscribe((res) => {
      this.loaiCV = res;
      this.loadList();
  ...
  filterConfiguration(): any {
    const filter: any = {};
    filter.loaicongviec = this.loaiCV;
    if (this.status) {
      if (this.status == 'New') {
        filter.isNew = 1;
      }
  ...
  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height26").clientHeight;
    tmp_height = tmp_height - 120;
    return tmp_height + "px";
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height26").clientWidth;
  ...
  loadList() {
    ...
    this.isLoading = true;
    this._WidgetCVQTService.loadList(queryParams).subscribe(res => {
  ...
  loadDataList_Lazy() {
    if (!this._loading) {
      ...
      if (this.crr_page < this.total_page) {
        ...
        this.isLoading = true;
        this._WidgetCVQTService.loadList(queryParams)
          .pipe(
