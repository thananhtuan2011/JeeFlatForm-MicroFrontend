...
@Component({
  selector: "my-works-widget-22",
  templateUrl: "./my-works-widget-22.component.html",
  styleUrls: ["./my-works-widget-22.component.scss"],
})
export class MyWorksWidget22Component
  implements
  OnInit,
  OnDestroy {
  ...
  constructor(
    ...
    public _WidgetWork22Service: WidgetWork22Service,
  ...
  ngOnInit() {
    ...
    this.btnFilterTag = this.btnFilterTag22.subscribe((res) => {
  ...
  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height22").clientHeight;
    tmp_height = tmp_height - 120;
    return tmp_height + "px";
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height22").clientWidth;
  ...
  loadList() {
    ...
    this.isLoading = true;
    this._WidgetWork22Service.loadList(queryParams).subscribe(res => {
  ...
  loadDataList_Lazy() {
    if (!this._loading) {
      ...
      if (this.crr_page < this.total_page) {
        ...
        this.isLoading = true;
        this._WidgetWork22Service.loadList(queryParams)
          .pipe(
