...
import { Widget18Service } from "../../Services/widget-18.service";
import { QueryParamsModelNew } from "src/app/_metronic/core/models/pagram";

@Component({
  selector: "widget-18",
  templateUrl: "./widget-18.component.html",
})
export class BieuDoWidget18Component
  implements
  OnInit,
  OnDestroy {
  ...
  btnFilterDK_Widget18: Subscription;
  btnDeleteFilterDK_Widget18: Subscription;
  btnThietlap_Widget18: Subscription;
  ...
  ListProject: any = [];
  @Input()
  campaign18StartEvent: EventEmitter<any> = new EventEmitter<any>();
  campaign18StartSub: Subscription;
  campaign18EndEvent: EventEmitter<any> = new EventEmitter<any>();
  campaign18EndSub: Subscription;
  campaign18RangeEvent: EventEmitter<any> = new EventEmitter<any>();
  campaign18RangeSub: Subscription;
  btnFilterEventTrangThaiCV18: EventEmitter<any> = new EventEmitter<any>();
  filterDSTrangThaiduan18: EventEmitter<any> = new EventEmitter<any>();
  btnLoaiBieuDo18: EventEmitter<any> = new EventEmitter<any>();

  filterDieuKienLoc_Widget18: EventEmitter<any> = new EventEmitter<any>();

  btnDeleteFilterEventCondition_Widget18: EventEmitter<any> = new EventEmitter<any>();
  btnThietlapFilter_Widget18: EventEmitter<any> = new EventEmitter<any>();

  btnFilterEventCondition_Widget18: EventEmitter<any> = new EventEmitter<any>();
  ...
  public barChartColors: Array<any> = [
   ...
   //=========================================================================
   btnFilterEventVaiTro18: EventEmitter<string> = new EventEmitter<string>();
   btnFilterSubVaiTro18: Subscription;
   filterDanhSachVaiTro18: EventEmitter<any> = new EventEmitter<any>();
   ListVaiTro: any[] = [];
   idRole: number = -1;
  constructor(
    private fb: FormBuilder,
    private changeDetectorRefs: ChangeDetectorRef,
    public _Widget18Service: Widget18Service,
  ...
  ngOnInit() {
    ...
    this.btnFilterSub = this.btnFilterEventTrangThaiCV18.subscribe((res) => {
      if (res != this.Team) {
        this.Team = res;
        setTimeout(() => {
        ...
        this.filterDSTrangThaiduan18.emit(object);
      }
    });

    this.btnFilterSub = this.btnLoaiBieuDo18.subscribe((res) => {
      this.LoaiBD = res;
      this.loadList(query);
      this.changeDetectorRefs.detectChanges();
    });
    this.campaign18RangeSub = this.campaign18RangeEvent.subscribe((res) => {
      this.selectedDate = res;
      this.campaign18EndSub = this.campaign18EndEvent.subscribe((res) => {
    ...
    this.btnFilterDK_Widget18 = this.btnFilterEventCondition_Widget18.subscribe((res) => {
      if (res != this.idDK) {
        this.idDK = res.RowID;
        ...
        this.filterDieuKienLoc_Widget18.emit(object);
      }
    });
    this.btnDeleteFilterDK_Widget18 = this.btnDeleteFilterEventCondition_Widget18.subscribe((res) => {
      this.delete_dk(res);

    });
    this.btnThietlap_Widget18 = this.btnThietlapFilter_Widget18.subscribe((res) => {
      this.load_đk();
      setTimeout(() => {
        this.loadList(query);
      }, 1000);
    ...
    this.btnFilterSubVaiTro18 = this.btnFilterEventVaiTro18.subscribe((res) => {
      this.idRole = +res;
      this.loadList(query);
      var object = {
      ...
      this.filterDanhSachVaiTro18.emit(object);
    });

    this.changeDetectorRefs.detectChanges();
  }
  load_đk() {
    this._WorkGeneralService.Get_listCustomWidgetByUser(18).subscribe(res => {
      if (res && res.status == 1) {
  ...
  pieChart(query) {
    this._Widget18Service.loadList(query).subscribe(res => {
      if (res && res.status == 1) {
  ...
  ngOnDestroy() {
    ...
    this.campaign18RangeSub.unsubscribe();
  }
  getClasses(val) {
