...
@Component({
  selector: "works-by-project-v1",
  templateUrl: "./works-by-project-v1.component.html",
})
export class WorksByProjectV1Component implements OnInit, OnDestroy {
  @Input() cssClass: "";
  ListProject: any = [];
  Team = 0;
  UserID = 0;
  ...
  btnFilterEventCongviecDuan40: EventEmitter<string> = new EventEmitter<string>();
  btnFilterEventThemCongViecDuan40: EventEmitter<string> =
    new EventEmitter<string>();
  filterDanhSachDuAn40: EventEmitter<any> = new EventEmitter<any>();
  isgov: boolean = false;
  //=====================================================================
  btnPriority40: EventEmitter<any> = new EventEmitter<any>();
  btnFilterSub40: Subscription;
  Priority = 0;
  //=========================================================================
  btnFilterEventVaiTro40: EventEmitter<string> = new EventEmitter<string>();
  btnFilterSubVaiTro40: Subscription;
  filterDanhSachVaiTro40: EventEmitter<any> = new EventEmitter<any>();
  ListVaiTro: any[] = [];
  idRole: number = -1;
  constructor(
  ...
  ngOnInit(): void {
    this.btnFilterSub = this.btnFilterEventCongviecDuan40.subscribe((res) => {
      this.Team = +res;
      setTimeout(() => {
      ...
      this.filterDanhSachDuAn40.emit(object);
    });
    this.btnFilterSubThem = this.btnFilterEventThemCongViecDuan40.subscribe(
      (res) => {
        this.Themmoi();
      }
    );

    this._JeeWorkLiteService.ListAllStatusDynamic().subscribe((res) => {
      if (res && res.status == 1) {
    ...
    this._JeeWorkLiteService.lite_project_by_manager().subscribe(
      (res) => {
        if (res && res.status == 1) {
          ...
          this.filterDanhSachVaiTro40.emit(object_role);
        }
      }
    );

    this.btnFilterSubVaiTro40 = this.btnFilterEventVaiTro40.subscribe((res) => {
      this.idRole = +res;
      this.loadList();
      var object = {
        list: this.ListVaiTro,
      ...
      this.filterDanhSachVaiTro40.emit(object);
    });

    this.btnFilterSub40 = this.btnPriority40.subscribe((res) => {
      this.Priority = res;
      this.loadList();
      this.changeDetectorRefs.detectChanges();
    });
  ...
  Themmoi() {
    if (this.isgov) {
      ...
      this.router.navigate(['', { outlets: { auxNameV1: 'auxWorkV1/createTask/0' }, }]);
    }
  }

  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height40").clientHeight;
    tmp_height = tmp_height - 120;
    return tmp_height + "px";
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height40").clientWidth;
  ...
  RemoveTag(tag, item) {
    ...
    this._JeeWorkLiteService.UpdateByKey(model).subscribe((res) => {
      if (res && res.status == 1) {
        this.loadList();
      } else {
  ...
  UpdateByKey(task, key, value) {
    ...
    this._JeeWorkLiteService.UpdateByKey(item).subscribe((res) => {
      if (res && res.status === 1) {
        this.loadList();
      } else {
        this.layoutUtilsService.showError(res.error.message);
  ...
  list_Tag = [];
  mark_tag(val) {
    this._JeeWorkLiteService.lite_tag(val.id_project_team).subscribe((res) => {
      if (res && res.status === 1) {
  ...
  statusChange(val) {
    this.listDataStatus = [];
    this._JeeWorkLiteService.ListStatusDynamicNew(val.id_project_team, val.id_row).subscribe(res => {
      if (res && res.status == 1) {
  ...
  loadDanhSachVaiTro() {
    ...
    if (+this.idRole == -1) {
      this.idRole = this.ListVaiTro[0].id;
    }
    var object = {
      list: this.ListVaiTro,
      name: this.NameofRole(),
