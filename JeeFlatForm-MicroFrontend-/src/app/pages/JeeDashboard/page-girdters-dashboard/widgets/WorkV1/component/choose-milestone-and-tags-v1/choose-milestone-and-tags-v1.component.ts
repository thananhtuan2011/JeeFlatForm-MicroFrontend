...
import { JeeWorkLiteService, TagsService } from '../../services/wework.services';

@Component({
	selector: 'kt-choose-milestone-and-tags-v1',
	templateUrl: './choose-milestone-and-tags-v1.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush,
})

export class ChooseMilestoneAndTagV1Component implements OnInit, OnChanges {
	// Public properties
	@Input() options: any[] = [];
	...
	constructor(
		...
		private _JeeWorkLiteService: JeeWorkLiteService,
	) { }
	...
	list_Tag: any = [];
	LoadTag() {
		this._TagsService.lite_tag(this.id_project_Team).subscribe((res) => {
			this.changeDetectorRefs.detectChanges();
			if (res && res.status === 1) {
				this.list = res.data;
				this.setUpDropSearchTags();
	...
	selected(id_milestone) {
		...
		this._JeeWorkLiteService.UpdateByKey(this.model).subscribe((res) => {
			if (res && res.status == 1) {
	...
	UpdateTag(color, _item, withBack: boolean = false) {
		...
		this._TagsService.Update(tagItem).subscribe(res => {
			if (res && res.status === 1) {
	...
	Create() {
		...
		this._TagsService.Insert(ObjectModels).subscribe(res => {
			this.changeDetectorRefs.detectChanges();
			if (res && res.status === 1) {
	...
	Delete(id) {
		this._TagsService.Delete(id).subscribe(res => {
			if (res && res.status === 1) {
				let item = {
					id_row: id,
					title: "delete"
