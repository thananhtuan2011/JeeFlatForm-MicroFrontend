...
@Component({
  selector: "trang-thai-cong-viec-widget-v1",
  templateUrl: "./trang-thai-cong-viec-widget-v1.component.html",
})
export class TrangThaiCongViecWidgetV1Component
  implements
  OnInit,
  OnDestroy {
  ...
  @Input()
  btnFilterEventTrangThaiCV39: EventEmitter<any> = new EventEmitter<any>();
  filterDSTrangThaiduan_Widget39: EventEmitter<any> = new EventEmitter<any>();
  btnLoaiBieuDo39: EventEmitter<any> = new EventEmitter<any>();
  filterDieuKienLoc_Widget39: EventEmitter<any> = new EventEmitter<any>();

  btnDeleteFilterEventCondition_Widget39: EventEmitter<any> = new EventEmitter<any>();
  btnThietlapFilter_Widget39: EventEmitter<any> = new EventEmitter<any>();

  btnFilterEventCondition_Widget39: EventEmitter<any> = new EventEmitter<any>();
  ...
  btnFilterEventVaiTro39: EventEmitter<string> = new EventEmitter<string>();
  btnFilterSubVaiTro39: Subscription;
  filterDanhSachVaiTro39: EventEmitter<any> = new EventEmitter<any>();
  ListVaiTro: any[] = [{
  ...
  ngOnInit() {
    ...
    this.btnFilterSub = this.btnFilterEventTrangThaiCV39.subscribe((res) => {
      if (res != this.Team) {
        this.Team = res;
        setTimeout(() => {
        ...
        this.filterDSTrangThaiduan_Widget39.emit(object);
      }
    });

    this.btnFilterSub = this.btnLoaiBieuDo39.subscribe((res) => {
      this.LoaiBD = res;
      this.loadList(query);
      this.changeDetectorRefs.detectChanges();
    });
    this.weworkService.lite_project_by_manager().subscribe(
      (res) => {
        if (res && res.status == 1) {
    ...
    this.btnFilterDK_TrangThaiCV = this.btnFilterEventCondition_Widget39.subscribe((res) => {
      if (res != this.idDK) {
        this.idDK = res.RowID;
        ...
        this.filterDieuKienLoc_Widget39.emit(object);
      }
    });
    this.btnDeleteFilterDK_TrangThaiCV = this.btnDeleteFilterEventCondition_Widget39.subscribe((res) => {
      this.delete_dk(res);

    });
    this.btnThietlap_TrangThaiCV = this.btnThietlapFilter_Widget39.subscribe((res) => {
      this.load_đk();
    ...
    this.btnFilterSubVaiTro39 = this.btnFilterEventVaiTro39.subscribe((res) => {
      this.idRole = +res;
      this.loadList(query);
      var object = {
      ...
      this.filterDanhSachVaiTro39.emit(object);
    });

    this.changeDetectorRefs.detectChanges();
  }
  load_đk() {
    this.weworkService.Get_listCustomWidgetByUser(39).subscribe(res => {
      if (res && res.status == 1) {
