...
import { TagsService } from '../../services/wework.services';
@Component({
  selector: 'kt-cu-tag-v1',
  templateUrl: './cu-tag-v1.component.html',
  styleUrls: ['./cu-tag-v1.component.scss']
})
export class CuTagV1Component implements OnInit {
  ...
  Update(_item: TagsModel, withBack: boolean) {
    this._TagsService.Update(_item).subscribe(res => {
      if (res && res.status === 1) {
  ...
  Create(_item: TagsModel, withBack: boolean) {
    this._TagsService.Insert(_item).subscribe(res => {
      if (res && res.status === 1) {
