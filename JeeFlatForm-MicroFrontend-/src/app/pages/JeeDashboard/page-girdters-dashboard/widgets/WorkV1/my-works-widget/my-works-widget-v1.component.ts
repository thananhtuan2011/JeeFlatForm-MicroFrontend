...
@Component({
  selector: "my-works-widget-v1",
  templateUrl: "./my-works-widget-v1.component.html",
  styleUrls: ["./my-works-widget-v1.component.scss"],
})
export class MyWorksWidgetV1Component
  implements
  OnInit,
  OnDestroy
  {
  ...
  @Input() cssClass: "";
list_widgets: any[] = [];
btnTienDo38: EventEmitter<any> = new EventEmitter<any>();
...
ngOnInit() {
  this.LoadData();
  this.btnFilterSub = this.btnTienDo38.subscribe((res) => {
    this.TienDo = res;
    this.loadList();
...
LoadData() {
  this._WidgetWorkService.ListAllStatusDynamic().subscribe((res) => {
    if (res && res.status === 1) {
...
getHeight(): any {
  let tmp_height = document.getElementById("gridster-height38").clientHeight;
  tmp_height = tmp_height - 120;
  return tmp_height + "px";
}
getWidth(): any {
  let tmp_width = document.getElementById("gridster-height38").clientWidth;
...
RemoveTag(tag, item) {
  ...
  this._WidgetWorkService.UpdateByKey(model).subscribe((res) => {
    if (res && res.status == 1) {
      this.loadList();
    } else {
...
UpdateByKey(task, key, value) {
  ...
  this._WidgetWorkService.UpdateByKey(item).subscribe((res) => {
    if (res && res.status === 1) {
      this.loadList();
    } else {
      this.layoutUtilsService.showError(res.error.message);
