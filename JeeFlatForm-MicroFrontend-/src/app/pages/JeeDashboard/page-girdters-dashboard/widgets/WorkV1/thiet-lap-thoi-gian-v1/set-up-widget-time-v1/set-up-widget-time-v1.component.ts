...
import { MotanoidungSetTimeV1Component } from "../motanoidung-set-time-v1/motanoidung-set-time-v1.component";
@Component({
  selector: 'app-set-up-widget-time-v1',
  templateUrl: './set-up-widget-time-v1.component.html',
  styleUrls: ['./set-up-widget-time-v1.component.scss']
})
export class SetUpWidgetTimeV1Component implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<SetUpWidgetTimeV1Component>,
  ...
  Detail_custom() {
    this.weworkService.Detail_custom(this.data.id_custom).subscribe(res => {
      if (res && res.status == 1) {
  ...
  loaddieukien() {
    this.weworkService.Get_listCustom().subscribe(res => {
      if (res && res.status == 1) {
  ...
  Thongtin() {
    ...
    const dialogRef = this.dialog.open(MotanoidungSetTimeV1Component, { data: { item }, width: '700px', panelClass: 'sky-padding-0' });
  ...
  Onsubmit(withBack: boolean) {
    if (this.Date_ && this.Operators && this.CustomID) {
      ...
      if (this.data.id_custom != 0) {
        this.weworkService.Update_custom_widget(item).subscribe(res => {
          if (res && res.status == 1) {
            this.dialogRef.close();
          }
        })
        ...
        this.weworkService.Insert_custom_widget(item).subscribe(res => {
          if (res && res.status == 1) {
            if (withBack) {
              this.dialogRef.close();
            }
