...
import { WorkDraftService } from "../../Services/list-works-draft.service";
...
@Component({
  selector: "list-works-draft",
  templateUrl: "./list-works-draft.component.html",
  styleUrls: ["./list-works-draft.component.scss"],
})
export class ListWorksDraftComponent
  implements
  OnInit,
  OnDestroy {
  ...
  constructor(
    ...
    public _workDraftService: WorkDraftService,
  ...
  onEventHandler(e: CustomEvent) {
    if (e.detail.eventType === 'load-draff') {
      this.loadList();
    }
  }
  LoadTatca() {
  ...
  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height32").clientHeight;
    //
    tmp_height = tmp_height - 120;
    return tmp_height + "px";
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height32").clientWidth;
    //
  ...
  loadList() {
    ...
    this.isLoading = true;
    this._workDraftService.loadList(queryParams).subscribe(res => {
      ...
      if (res && res.status == 1) {
        ...
        console.log("data lazy load", this.dataLazyLoad);
  ...
  loadDataList_Lazy() {
    if (!this._loading) {
      ...
      if (this.crr_page < this.total_page) {
        ...
        this.isLoading = true;
        this._workDraftService.loadList(queryParams)
          .pipe(
  ...
  Add(id_row) {
    this.router.navigate(['', { outlets: { auxName: `auxWork/AddGOV/${id_row}`}, }]);
  }

  Update(_item: WorkModel, id_row) {
    setTimeout(() => {
      // const dialogRef = this.dialog.open(CongViecTheoDuAnVer1PopupComponent, { data: { _item, _type: "0", idDraft: id_row }, panelClass: ['sky-padding-0', 'width900'], disableClose: true });
