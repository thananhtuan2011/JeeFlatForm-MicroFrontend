...
import { WidgetTheoDoiNhiemVuDaGiaoService } from '../../Services/widget-theo-doi-nhiem-vu-da-giao.service';
...
@Component({
  selector: 'widget-theo-doi-nhiem-vu-da-giao',
  templateUrl: './widget-theo-doi-nhiem-vu-da-giao.component.html',
})
export class WidgetTheoDoiNhiemVuDaGiaoComponent
  implements
  OnInit,
  OnDestroy {
  ...
  @Input()
  //================================================================
  btnFilterEventDepartment27: EventEmitter<string> = new EventEmitter<string>();
  btnFilterSub: Subscription;
  idPb: number = 0;
  filterDanhSachPhongBan27: EventEmitter<any> = new EventEmitter<any>();
  ListPhongban: any[] = [];
  is_loading = false;
  constructor(
    private fb: FormBuilder,
    private changeDetectorRefs: ChangeDetectorRef,
    public _WidgetTheoDoiNhiemVuDaGiaoService: WidgetTheoDoiNhiemVuDaGiaoService,
  ...
  ngOnInit() {
    ...
    this.btnFilterSub = this.btnFilterEventDepartment27.subscribe((res) => {
      this.idPb = +res;
      if (this.idPb != 0) {
        this.loadList();
      ...
      this.filterDanhSachPhongBan27.emit(object);
    });
    if (this.idPb != 0) {
      this.loadList();
      this.is_loading = true;
  ...
  loadList() {
    ...
    this._WidgetTheoDoiNhiemVuDaGiaoService.loadList(query).subscribe(res => {
      if (res && res.status == 1) {
        resItems = res.data;
        let sum = {
  ...
  getHeight(): any {
    let tmp_height = document.getElementById("gridster-height27").clientHeight;
    tmp_height = tmp_height - 120;
    return tmp_height + 'px';
  }
  getWidth(): any {
    let tmp_width = document.getElementById("gridster-height27").clientWidth;
