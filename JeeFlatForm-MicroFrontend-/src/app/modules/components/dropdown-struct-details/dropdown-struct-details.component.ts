import { Component, ViewEncapsulation, Input, Output, EventEmitter, forwardRef, OnChanges, ChangeDetectorRef, ViewChild, ElementRef } from '@angular/core';
...
import { DataItem } from '../dropdown-struct-hr-details/dropdown-struct-hr-details.component';
...
@Component({
  selector: 'm-dropdown-struct-details',
  templateUrl: './dropdown-struct-details.component.html',
  styleUrls: ['./dropdown-struct-details.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [NgbDropdownConfig,
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => DropdownStructDetailsComponent),
      multi: true
    },
    {
...
export class DropdownStructDetailsComponent implements OnChanges, ControlValueAccessor, Validator {
  ...
  SelectItemTree(node: TreeNode) {
    if (this.IsLoad && +node.RowID > 0) {
      ...
      if (this.selectedNode && this.selectedNode.DepartmentName) {
        this.isValid = true;
      }
      else {
        this.isValid = false;
      }
      this.dropdowntreeControl.setValue(this.selectedNode.DepartmentName);
  ...
  getTitle(data, ID) {
    data.forEach(element => {
      if (element.RowID == ID) {
        this.selectedNode.DepartmentName = element.DepartmentName;
        return;
      }
    ...
    this._DropdownStructControlService.Get_Text(id_ward)
      .then(res => {
        if (res.status == 1 && res.data.length > 0) {
          ...
          this.selectedNode = {
            DepartmentName: res.data[0].Title,
            RowID: +id_ward
          };
    ...
    if (filterText) {
      ...
      function filter(array: TreeNode[], text: string) {
        const getChildren = (result: TreeNode[], object: TreeNode) => {
          if (object.DepartmentName.toLowerCase().includes(text.toLowerCase())) {
            result.push(object);
            return result;
          }
