...
import { UpdateStatusWFDialogComponent } from '../update-status-dialog-wf/update-status-dialog-wf.component';
import { ConfirmConfigDialogComponent } from '../action-confirm-config/action-confirm-config.component';
...
export class LayoutUtilsService {
   ...
   deleteElement(title: string = '', description: string = '', waitDesciption: string = '', doPositiveBtn: string = 'Delete',isDel:boolean = true ) {
	...
	updateStatusForCustomersNew(title: string = '', description: string = '', waitDesciption: string = '') {
		return this.dialog.open(UpdateStatusWFDialogComponent, {
	...
	//================Start Thiên code 12/07/2023 (chỉ sử dụng 1 lần)================
	OnWaitingRouter() {
		let v_idWaiting: string = 'nemo-router-waiting-id';//id waiting
		let v_idWaitingLoader: string = 'nemo-router-waiting-loader';//id waiting
		let _show: string = 'nemo-show-wait';
		let _hide: string = 'nemo-hide-wait';
		let divWait = document.getElementById(v_idWaiting);
		let loader = document.getElementById(v_idWaitingLoader);
	...
	//================End Thiên code 12/07/2023 (chỉ sử dụng 1 lần)================

	//================Start sử dụng cho lần khởi tạo đầu tiên user admin app========
	public Confirm_APP(
        title: string,
        message: string,
        btnLater: string = '',
        btnAgree: string = '',
        dialogSize: 'sm' | 'lg' = 'sm'): Promise<boolean> {
        const modalRef = this.modalService.open(ConfirmConfigDialogComponent, 
		{ size: '280px', backdrop: 'static', keyboard: false });
        modalRef.componentInstance.title = title;
        modalRef.componentInstance.message = message;
        modalRef.componentInstance.btnLater = btnLater;
        modalRef.componentInstance.btnAgree = btnAgree;
        return modalRef.result;
    }
}
